...
import { useOpenPO, labelToSlug, extractMonthKey } from "../../hooks/useOpenPoSummary";
import { exportToXLSX, exportToPDF } from '../../utils/poExport';
import { useOpenPoStore } from "../../stores/openPoStore";
...
// ─── Year pill ───────────────────────────────────────────────────────────────
function YearPill({ years, value, onChange }) {
  return (
      ...
      {years.map((y) => (
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-semibold transition-all cursor-pointer
            ${value === y ? "bg-gray-900 text-white shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          FY{y}
        </button>
      ))}
...
// ─── Main component ──────────────────────────────────────────────────────────
export default function OpenPoSummary({
  ...
  // Destructure grandTotals from hook
  const { rows, buyerList, vendorList, monthLabels, totalPages, totalCustomers, overall, loading, error, fetchAllForExport } =
  ...
  const handleExport = async (type) => {
    ...
    try {
      const rawRows = await fetchAllForExport({
        year,
        buyer: buyer,
        month: monthSlugForApi,
        vendor,
      })
      const allRows = rawRows.filter(matchesClientFilters)
      const label = `Open_PO_FY${year}`
      if (type === 'xlsx') {
        exportToXLSX(allRows, `Open PO FY${year}`, label)
      } else {
        await exportToPDF(allRows, `Open Purchase Orders — FY 20${year}`, label)
      }
    } finally {
      setExporting(false)
    }
  ...
  const totals = useMemo(() => ({
    ...
    openPOs:   new Set(filteredRows.map((r) => `${r.vendor}__${r.poNo}`)).size,
  ...
  // Server already returns only the current page's customers — no client slicing needed
  const cusKeys      = useMemo(() => Object.keys(grouped).sort(), [grouped]);
  const pagedCusKeys = cusKeys;
  ...
  // ── build table rows (memoized — only rebuilds when data/collapse/sort changes) ──
  const { tableRows, grandTotals } = useMemo(() => {
              ...
              {COLS.map((c) => {
                ...
                if (c.sticky) return (
                  <td key={c.key} className={`sticky left-0 z-[2] bg-blue-50 hover:bg-blue-100 font-semibold text-[13px] ${base} ${c.w}`}>
                    <svg
                      className={`inline w-4 h-4 mr-1.5 text-blue-500 align-middle transition-transform duration-150 ${col ? "" : "rotate-90"}`}
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                    >
              ...
              rows.push(
                <tr key={`d-${gk}-${idx}`} className={`${rowBg} hover:bg-blue-50 transition-colors`}>
                  {COLS.map((c) => {
                    ...
                    return (
                      <td key={c.key} className={`${base} text-[13px] text-gray-700 ${c.w}`}>
                        {r[c.key] || ""}
                      </td>
                    );
                  })}
    ...
    return {
      tableRows: rows,
      grandTotals: { grandOV, grandSV, grandBV, grandOQ, grandSQ, grandBQ, grandBiz },
    };
  }, [pagedCusKeys, grouped, collapsed, sortedRows, togglePO]);
  
  // Add this back after the tableRows useMemo
useEffect(() => { setPage(1) }, [vendor, monthFilter, search])
    ...
    <div className="font-sans text-gray-800 bg-gray-50">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base font-semibold text-gray-900 leading-tight">Open Purchase Orders</div>
            <div className="text-xs text-gray-400 mt-0.5">FY 20{year} · Live data</div>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <select
            ...
            onChange={e => {
              ...
              setVendor(""); setMonthFilter(""); setSearch("")
              setCollapsed(new Set()); setAllExpanded(true)
            }}
      ...
      {/* Chips — always show grand totals from server */}
